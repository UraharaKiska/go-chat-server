...
func (m *MessageInfo) validate(all bool) error {
	...
	if l := utf8.RuneCountInString(m.GetFrom()); l < 2 || l > 50 {
	...
	if l := utf8.RuneCountInString(m.GetText()); l < 2 || l > 50 {
		err := MessageInfoValidationError{
			field:  "Text",
			reason: "value length must be between 2 and 50 runes, inclusive",
		}
...
func (m *CreateRequest) validate(all bool) error {
	...
	if all {
		switch v := interface{}(m.GetChatInfo()).(type) {
		case interface{ ValidateAll() error }:
			if err := v.ValidateAll(); err != nil {
	...
	} else if v, ok := interface{}(m.GetChatInfo()).(interface{ Validate() error }); ok {
		if err := v.Validate(); err != nil {
